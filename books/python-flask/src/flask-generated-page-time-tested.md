...
Putting them in separate functions allows us to run them separately. It also means that if one fails the other might still pass. Usually we put independent test-cases in separate functions.
...
The `test_time` function is trickier. We can't check an exact match as the timestamp will be different every time we run the code. We could **mock** the time and for a real application I'd probably do that, but for now we are looking for a simpler solution. So instead of an exact match we use a Regular Expression (a regexp) to check if the result looks like a number we would expect.

You can run the tests by running `pytest`.

## The test:

{% embed include file="src/examples/flask/time/test_show_time.py" %}
